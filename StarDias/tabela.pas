...
uses
  ...
  StdCtrls, Forms, DBCtrls, DB, DBGrids, DBTables, Grids, ExtCtrls,Printers, pomoc,
  SAPLogonCtrl_TLB, OleCtrls, SAPFunctionsOCX_TLB, Menus, variants, casiSt,sinapro,prijavaSAP,deklar, System.Generics.Collections;
...
type
  ...
  TFtabela = class(TForm)
    ...
    Orodja1: TMenuItem;
    Konec1: TMenuItem;
    Tabelapredpisanihasov1: TMenuItem;
    Pridobipredpisanias1: TMenuItem;
    Analizazaendan1: TMenuItem;
    Brwezpodatkov1: TMenuItem;
    Brezpei1: TMenuItem;
    Vpispei1: TMenuItem;
    Briivrstico1: TMenuItem;
    tnaloga1: TMenuItem;
    Najdinalog1: TMenuItem;
    Koda1: TMenuItem;
    BriiVe1: TMenuItem;
    PrijavaVSAP1: TMenuItem;
  ...
  //  procedure Button8Click(Sender: TObject);
    procedure Tabelapredpisanihasov1Click(Sender: TObject);
    procedure Pridobipredpisanias1Click(Sender: TObject);
    procedure Analizazaendan1Click(Sender: TObject);
    procedure Brwezpodatkov1Click(Sender: TObject);
    procedure Brezpei1Click(Sender: TObject);
    procedure Vpispei1Click(Sender: TObject);
    procedure Konec1Click(Sender: TObject);
    procedure Briivrstico1Click(Sender: TObject);
    procedure tnaloga1Click(Sender: TObject);
    procedure Najdinalog1Click(Sender: TObject);
    procedure Koda1Click(Sender: TObject);
    procedure BriiVe1Click(Sender: TObject);
    procedure PrijavaVSAP1Click(Sender: TObject);
    procedure Table1DATUMChange(Sender: TField);
  private
    dst : TdateTime ;
  ...
  public
    ...
    Function CheckPO(porder : string ) : boolean ;
    procedure GetProdDetail(connection : variant;deln : string; var tabc : variant; var vredu : boolean; var mat : string) ;
    Function KodaConv(connection : variant;koda : string; cc : char) : string ;
    procedure getDonos(connection : variant; stnal,conf : string;  var yield,scrap : longint) ;
  end;
...
function TfTabela.DatToText(dt : TdateTime) : string ;
  var nz,d1,d2 : string ;
      yr,mo,da : word ;
begin
  decodedate(dt,yr,mo,da) ;
  nz := IntToStr(yr) ;
  d1 := IntTostr(mo) ;
...
// procedura za prenos podatkov iz SAP-a
Procedure  TfTabela.prenos(dat : TdateTime;ix : Integer)  ;
    var i,jj,k1,i1,t1,t2,t3,pc,tz,ii,j : Integer ;
      connection : variant ;
      tab,tabb,funct,funct1,funct2,funct3,tabc,tabv : variant ;
      snal,koda,wcen,st_potr,st_nal,mrp,dtt,kd : string ;
      don,nn,db,n1,n2,n3,n4 :integer ;
      bcen : boolean ;
      tt : longint ;
      arbid,skl,cc,pp,sskl,pk : string ;
      kk : longint ;
      plist : Tlist ;
      poi : ^stornis ;
      prv : boolean ;
   ...
   begin
     if Table1.Locate('potrditev;donos',VarArrayOf([st_potr,don]),[]) then result := false
                                                                     else result := true ;
   end ;

   function correct(stpotr : string; don : Integer)  : boolean ;
   begin
     if Table1.Locate('potrditev;donos',VarArrayOf([stpotr,don]),[]) then
     begin
       table1.delete ;
       result := true ;
     end else
   ...
   begin
     ...
     begin
       dbErr := EDBEngineError(E);
       for logIdx := 0 to dbErr.ErrorCount - 1 do
         PisiLog('MACHINE=' + vMachine + '; STEP=' + AStep +
                 '; DB_ERR_CODE=' + IntToStr(dbErr.Errors[logIdx].ErrorCode) +
                 '; DB_ERR_MSG=' + dbErr.Errors[logIdx].Message);
     end;
   end;
...
begin
  ...
  begin
     ...
     LogStep('VALIDATION', 'Prenos za današnji dan ni mogoč');
     exit
  end ;
  ...
  try
    ...
    // 1) Try NON-exclusive open first (diagnostic)
    Table1.Exclusive := False;
    try
      Table1.Open;   // if THIS fails -> it's not “exclusive problem”, it's path/file/permission
      Table1.Close;

      // 2) Now try exclusive open
      Table1.Exclusive := True;
      try
        Table1.Open;
      except
      ...
      on E: Exception do
      begin
        LogDbError('OPEN_NON_EXCLUSIVE', E);
    ...
    begin
      ...
      begin
        if Trunc(Table1.FieldByName('DATUM').AsDateTime) = Trunc(dat) then
          Table1.Delete
        else
          Table1.Next;
      end;
    ...
    if ix = 1 then
    begin
      // preverim če je bil prenos že izvršen
      if Table1.Locate('datum',dat,[]) then
      begin
        ShowMessage('Prenos za ta dan je že izveden') ;
        LogStep('VALIDATION', 'Prenos za ta dan je že izveden');
        exit
      end ;
    end ;
    ...
    begin
      vLoggedOn := True;
      // preberem tabelo potrditev za določen datum
      ...
      begin
         ...
         begin
           ...
           if don > 0 then          // omejim se na potrditve ki imajo donos večji od 0
           begin
             // obravnava stornov
             ...
             // preverim če je podatek o kodi pravi
             ...
             begin
               ...
               if n4 > 0 then   // če je krivljenje je to pravi kriterij
               begin
               ...
               if nsm then
               begin
                  // pretvorba dolge kode v običajno kodo
                 { SAPFunctions1.Connection := Idispatch(Connection);
                  ...
                  skl := tabc.value(1,14) ;       // skladišče
                  pp := copy(koda,14,2);

                  if otip = 'R' then cc := 'Y'    // 100%
                    else  if (copy(skl,3,2) = '47') or (copy(skl,3,2) = '80') then cc := 'X' else cc := 'U' ;   // statistična
                  koda[1] := '0' ;
...
//                    ]);
              ...
              Table1.Append;
                      ...
                      Table1.FieldByName('NALOG').AsString     := Copy(st_nal,6,7);
                      Table1.FieldByName('KODA').AsString      := koda;
                      Table1.FieldByName('POTRDITEV').AsString := st_potr;
                      Table1.FieldByName('DONOS').AsFloat      := don;
                      Table1.FieldByName('DELMESTO').AsString  := wcen;
                      Table1.FieldByName('DATUM').AsDateTime   := dat;

                      Table1.FieldByName('DOBA').AsFloat       := db;
                      Table1.FieldByName('RAZLIKA').AsFloat    := 0;
                      Table1.FieldByName('TIP').AsString       := cc;
                      Table1.FieldByName('PEC').AsFloat        := pc;
                      Table1.FieldByName('SKLAD').AsString     := skl;

                      Table1.FieldByName('TK1').AsFloat        := t1;
                      Table1.FieldByName('TK2').AsFloat        := t2;
                      Table1.FieldByName('TK3').AsFloat        := t3;

                      Table1.FieldByName('AVTOMAT').AsFloat    := 0;
...
procedure TFtabela.Prijava(var connection : variant) ;
  var prija : Tprijava ;
begin
...
procedure TFtabela.PrijavaVSAP1Click(Sender: TObject);
begin
  FprijavaSAP.prikaz ;
end;

Function TFtabela.CheckPO(porder : string ) : boolean ;
  var nal : string ;
begin
  Table1.IndexName := 'nalog' ;
  Table1.Open ;
  nal := Copy(porder,6,7) ;
  If table1.Locate('nalog',nal,[]) then result := true else result := false ;
  Table1.Close ;
end ;


// prikaz tabele predpisanih časov
procedure TFtabela.Tabelapredpisanihasov1Click(Sender: TObject);
begin
  FCasiSt.pokazi ;
end;

procedure TFtabela.Table1DATUMChange(Sender: TField);
begin

end;

// omejitev na trenutni nalog
procedure TFtabela.tnaloga1Click(Sender: TObject);
  var nl : string[10] ;
begin
nl := Table1.FieldByName('NALOG').AsString;

  If not table1.Filtered then
  begin
    Table1.Filtered := true ;
    Table1.filter := 'nalog = ' + '''' + nl + '''' ; //+ ' 0 + '''' ;
  end else
  begin
    Table1.Filtered := false ;
    Table1.Filter := '' ;
...
// popravilo oziroma vpis predpisanega časa staranja
procedure TFtabela.Pridobipredpisanias1Click(Sender: TObject);
    var kd,sf,mater,kk,pk,dodkd,kdd : string ;
      db,t1,t2,t3,tt : integer ;
      kkon : sest ;
      connection, funct, tab, funct1 : variant ;
begin
  kdd := Table1.FieldByName('KODA').AsString;

  dodkd := copy(kdd,14,2) ;
  kd := copy(kdd,1,12) ;

  //  v bazi MPTER iščem po kodi
  FcasiSt.isci(kd,db,t1,t2,t3,tt,pk,kkon) ;
  if db <> 0 then
  begin
   // Novadoba(dodkd,kkon,db) ;
    table1.edit ;
    Table1.FieldByName('DOBA').AsFloat := db;
Table1.FieldByName('TK1').AsFloat  := t1;
Table1.FieldByName('TK2').AsFloat  := t2;
Table1.FieldByName('TK3').AsFloat  := t3;
    ...
    Table1.post ;
    FcasiSt.Zapis(kdd,'',pk,db,t1,t2,t3,tt) ;
  end ;
end;

// štetje potrditev za časovni interval
...
begin
  ...
  while not table1.eof do
  begin
    dd := Table1.FieldByName('DATUM').AsDateTime;
    if (dd >= dat1) and (dd <= dat2) then
    begin
      stev := round(Table1.FieldByName('DONOS').AsFloat) + stev ;
      Inc(nn) ;
      if Round(Table1.FieldByName('PEC').AsFloat) <> 0 then
      begin
        stv1 := stv1 + round(Table1.FieldByName('DONOS').AsFloat);
        Inc(n1)
      end ;
    end ;
    Table1.Next ;
...
procedure TFtabela.Brwezpodatkov1Click(Sender: TObject);
 var potr : string ;
begin
  If not table1.Filtered then
  begin
    Table1.Filtered := true ;
    Table1.filter := 'doba = 0';
  end else
  begin
    Table1.Filtered := false ;
    Table1.Filter := '' ;
...
// Procedura za eliminacijo večkratnih zapisov iste kode in avtomata
...
// omejitev na zapise ki nimajo podatke o peči
procedure TFtabela.Brezpei1Click(Sender: TObject);
begin
  If not table1.Filtered then
  begin
    Table1.Filtered := true ;
    Table1.filter := 'pec = 0' ; //+ ' 0 + '''' ;
  end else
  begin
    Table1.Filtered := false ;
    Table1.Filter := '' ;
...
// poskušam pridobiti podatke o peči
...
begin
  ...
  begin
    if Round(Table1.FieldByName('PEC').AsFloat) = 0 then
    begin
      kd := Copy(Table1.FieldByName('KODA').AsString, 1, 12);
...
procedure TFtabela.Konec1Click(Sender: TObject);
begin
  Close
end;

procedure TFtabela.Najdinalog1Click(Sender: TObject);
  var nl : string ;
begin
  nl := inputBox('Najdi nalog','','') ;
  Table1.Locate('nalog',nl,[]) ;
end;

// brisanje do konca tabele
procedure TFtabela.BriiVe1Click(Sender: TObject);
  var nr1,nr2 : integer ;
begin
  while not table1.eof do
  begin
...
// branje podatkov za delovni nalog
procedure TFtabela.GetProdDetail(connection : variant;deln : string; var tabc : variant; var vredu : boolean; var mat : string) ;
  var funct1 : variant ;
      j : integer ;
      op : string ;
begin
   ...
   begin
     ...
     mat :=  funct1.TABLES.item('HEADER').value(1,5) ;
     funct1.TABLES.item('HEADER').DeleteRow
   end;
end ;


// omejitev na trenutno kodo
procedure TFtabela.Koda1Click(Sender: TObject);
    var kd : string[15] ;
begin
  kd := Table1.FieldByName('KODA').AsString;
//  kd := '0055.817.057-00' ;
  If not table1.Filtered then
  begin
    Table1.Filtered := true ;
    Table1.filter := 'koda = ' + '''' + kd + '''' ; //+ ' 0 + '''' ;
  end else
  begin
    Table1.Filtered := false ;
    Table1.Filter := '' ;
...
Function TFtabela.KodaConv(connection : variant;koda : string; cc : char) : string ;
  var funct2 : variant ;
begin
   SAPFunctions1.Connection := Idispatch(Connection);
   Funct2 :=SAPFunctions1.add('ZRFC_CONV_MATNR_GET');
...
// donos in izmet za določeno potrditev
...
procedure TFtabela.PisiLog(logtxt : string) ;
  var ff : textfile ;
      imef : string ;
begin
  ...
  try
    try
      ...
      // log write failures must not stop business flow
    end;
  except
