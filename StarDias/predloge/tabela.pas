...
uses
  ...
  SAPLogonCtrl_TLB, OleCtrls, SAPFunctionsOCX_TLB, Menus, variants;
...
// procedura za prenos podatjkov iz SAP-a
  ...
  begin
    ...
    begin
       ...
       begin
         ...
         //if st_potr = '0027195821' then
           //      Showmessage('a') ;

         // omejim se na potrditve iz treh delovnih mest
         dm1 := (arbid = '10004954') or (arbid = '10004956') or (arbid = '10005372') or (arbid = '10006756') ;
         dm2 := (arbid = '10005289') or (arbid = '10005004') or (arbid = '10005112') or (arbid = '10005314');
         if dm1 or dm2 then
         begin
           ...
           if arbid = '10005314' then wcen := '4417-427' ;
           don := Tab.value(i,38);
           ...
           begin
             ...
             begin
               ...
               begin
                  ...
                  if (ssx = '4334-401') or (ssx = '4452-401') or (ssx = '4334-427') then  n1 := ii ;
                  if (ssx = '4391-401') or (ssx = '4076-401') or (ssx = '4170-401') or (ssx = '4417-427') then n2 := ii
                  {  begin
                      nsm := true ;
                      break ;
                    end ;  }
               ...
               if ((n1 < n2) and dm1) or ((n2 < n1) and dm2) then nsm := true else nsm := false ;
               ...
               begin
                  ...
                  pp := copy(koda,14,2);
                  if copy(wcen,1,4) = '4475' then cc := 'Y'  // 100%
                  else begin
                    if (pp = '01') or (pp = '11') or (pp = '21') then cc := 'Y'     // 100%
                    else
                      if (copy(skl,3,2) = '47') or (copy(skl,3,2) = '80') then cc := 'X' else cc := 'U' ;   // statistièna
                  end ;
                  koda[1] := '0' ;
                  ...
                  if prv then
                     Table1.AppendRecord([copy(st_nal,6,7),koda,st_potr,don,wcen,dat,db,0,cc,pc,skl,t1,t2,t3]) ;
                end ;
             end ;
...
begin
  ...
  begin
    ...
    table1tk3.value := t3 ;
    Table1.post ;
    FcasiSt.Zapis(kd,'',pk,db,t1,t2,t3,tt) ;
  end else
  begin
    // iskanje preko šifre v MPTER-u
    sf := copy(kd,6,3) + copy(kd,10,3) ;
    FcasiSt.isci1(sf,db,t1,t2,t3,tt,kk,pk) ;           // iskanje po šifri

    // èe je bilo iskanje uspešno podatek zapišem v tabelo casi.dbf
    if db <> 0 then
    begin
      table1.edit ;
      table1doba.value := db ;
      ...
      table1tk3.value := t3 ;
      Table1.post ;
      FcasiSt.Zapis(kd,kk,pk,db,t1,t2,t3,tt) ;
    end ;
  end ;
end;
...
procedure TFtabela.Analizazaendan1Click(Sender: TObject);
   var stev,nn,stv1,n1 : longint ;
      ...
      dt : string ;
begin
  // dat := Table1datum.value ;
  //dt := InputBox('Vpiši datum','',dateTostr(dat)) ;
...
begin
  dat := date - 30;
  Table1.filter := 'datum > ' + DateToStr(dat) ;
  Table1.Filtered := true ;
  Table1.First ;
  while not Table1.eof do
  begin
    //if Table1datum.value = dat then
    if Table1pec.value = 0 then
    begin
      kd := copy(Table1koda.value,1,12) ;
